...
"""
    simple_model_comparison.jl

Simple and direct model comparison between BNN-ODE and UDE models.
Evaluates using standard metrics: MSE, RMSE, MAE, R².
"""
...
println("🔬 SIMPLE MODEL COMPARISON")
println("=" ^ 50)
...
# Use a single scenario for evaluation (first one)
scenario = test_data.scenario[1]
scenario_data = test_data[test_data.scenario .== scenario, :]
sort!(scenario_data, :time)

println("  → Using scenario: $scenario")
println("  → Scenario samples: $(nrow(scenario_data))")
...
if isfile(bnn_path)
    ...
    # Define BNN-ODE dynamics function
...
println("\n" * "="^50)
println("QUANTITATIVE RESULTS SUMMARY")
println("="^50)
...
CSV.write("results/simple_model_comparison.csv", results_df)
println("  → Results saved to results/simple_model_comparison.csv")

println("\n✅ Simple model comparison completed!") 